(function() {
  ...
  /** @ngInject */
  function ConifgDeliveryController(
    $http,
    api,
    $scope,
    zipcodeService,
    ...
    function allStatesHaveTheSameValues(states) {
      ...
      for (var i = 0; i < size; i++) {
        if (hash) {
          if (
            hash !==
            states[i].deliveryPriceMode + '#' + states[i].deliveryPriceValue
          ) {
            return false;
          }
        } else {
    ...
    function updateZipcodeStates() {
      ...
      return zipcodeService
        .updateZipcodeStates({ zipcodeStates: zipcodeStates })
