(function() {
  ...
  angular.module("app.directives").directive("tableList", tableList);
  ...
  var controller = function(
    ...
    $scope.dtOptions = DTOptionsBuilder.newOptions()
      ...
      .withOption("initComplete", function() {
        if ($("#new-search").length <= 0) {
          $('<p class="sorting-by-label"></p>').appendTo(
            ".dataTables_wrapper .top"
          );
          $(".dataTables_wrapper .top .sorting-by-label").text(
            "Ordenado por: " + $scope.columns[0].label
          );
          $("<button/>")
            .text("Buscar")
            .attr("id", "new-search")
    ...
    function serverData(sSource, aoData, fnCallback, oSettings) {
      ...
      //Do not sort when is a destroy column
        ...
        !$scope.columns[sortingColumnIndex].destroy &&
        !$scope.columns[sortingColumnIndex].editUrl
      ) {
        query.orderby = sortingColumnName + " " + sortingDirection;
        $(".dataTables_wrapper .top .sorting-by-label").text(
          "Ordenado por: " + sortingColumnLabel
        );
      }
        ...
        DTColumnBuilder.newColumn(column.key)
          ...
          .renderWith(function renderCell(data, type, full, pos) {
            ...
            if (column.destroy) {
              ...
              var id = column.propId ? column.propId : "id";
              var icon =
                '<md-icon md-font-icon="icon-pencil" class="icon edit-pencil md-font icon-pencil material-icons md-default-theme" aria-hidden="true"></md-icon>';
